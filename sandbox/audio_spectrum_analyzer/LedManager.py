...
def fadeBars(fadeData, barsData, slowFade, fade_speed):
    fadedBars = [0] * 64
    index = 0
...
def run_pyaudio_fft_spectrum(obj, infinite=False):
    ...
    while True:
        ...
        for i, frequency in enumerate(binned_fftx):
            ...
            if max(binned_fft) > current_max:
                current_max = max(binned_fft)
            ...
            if power <= (max_value * 0.001):
                ...
                level = translate(power, 0, current_max, 0, 23)
                if level == None:
                    level = 0
                else :
                    level = int(level)
                
                multiplier = 0.8 - (int(i / 4) * 0.04)
                
                if level > (18 * multiplier):
                    level = 7
                elif level > (14 * multiplier):
                    level = 6
                elif level > (9 * multiplier):
                    level = 5
                elif level > (6 * multiplier):
                    level = 4
                elif level > (4 * multiplier):
                    level = 3
                elif level == (2 * multiplier):
                    level = 2
                elif level >= (1 * multiplier):
                    level = 1
                else:
                    level = 0
        ...
        fadedBars = fadeBars(fadedBars, barsData, slowFade, fade_speed)
